...
# Quick-access CTA in the sidebar
st.sidebar.markdown(f"[📅 Book a consultation]({SCHEDULING_LINK})")

# Helper to detect if user intends to schedule
def wants_scheduling(text: str) -> bool:
    if not text:
        return False
    t = text.lower()
    keywords = [
        "schedule", "scheduling", "book", "booking",
        "consult", "consultation", "meeting", "call",
        "appointment", "time slot", "calendar"
    ]
